...
/** isTestForDistribution = NO Apple测试服务器的推送通知
    isTestForDistribution = YES Apple正式服务器的推送通知
 */
@property(nonatomic, assign)BOOL isTestForDistribution;
#pragma mark Private
- (void)connect;
- (void)disconnect;
@end
...
- (id)init {
	...
	if(self != nil) {
		...
		self.deviceToken = @"b6cc0a75 24f9e557 bfbb5a25 c4e98629 d917b6cc aafc0f37 d7729fde c87da9c5";
        self.payload = @"{\"aps\":{\"sound\":\"default\",\"badge\":\"1\",\"alert\":\"吃饭了吗!\"}}";
        
		self.certificate = [[NSBundle mainBundle] pathForResource:@"aps_development" ofType:@"cer"];
        self.isTestForDistribution = NO;
	}
	return self;
}
...
- (void)connect {
    ...
    result =  (self.isTestForDistribution == YES) ? MakeServerConnection("gateway.push.apple.com", 2195, &socket, &peer) : MakeServerConnection("gateway.sandbox.push.apple.com", 2195, &socket, &peer);
    ...
    result = (self.isTestForDistribution == YES) ?  SSLSetPeerDomainName(context, "gateway.push.apple.com", 22) : SSLSetPeerDomainName(context, "gateway.sandbox.push.apple.com", 30);
